...
// ===== ESP32 Wi-Fi設定 =====
// Arduino IDEのシリアルモニターに出ていたESP32のIPアドレス
...
// ===== Wi-Fi状態表示 =====
let wifiStatus = "ESP32 Wi-Fi mode";
...
function setup() {
  ...
  // 起動時に一度だけESP32へIDLEを送る
  // ESP32側ではIDLE時にポンプOFF
  sendIdleCommand();
...
async function autoStartMic() {
  try {
    ...
    console.log("Auto mic start failed:", error);

    // 自動開始できなかった場合だけ、手動ボタンを出す
    createMicButton();
  }
}
...
function createMicButton() {
  ...
  micButton.style("touch-action", "manipulation");

  async function handleStartMic() {
    console.log("Start Mic button touched/clicked");
  ...
  // PC用
  micButton.mousePressed(handleStartMic);

  // iPad / タッチ用
  micButton.elt.addEventListener("touchstart", async (event) => {
...
async function startMic() {
  try {
    ...
    errorMessage = "マイクを自動開始できませんでした。Start Mic を押してください。";
...
function drawWaitingScreen() {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("マイク起動を待っています", 40, 90);

  fill(120);
  textSize(12);
  text(`ESP32 Wi-Fi: http://${ESP32_IP}`, 40, 120);
  text("自動起動できない場合は Start Mic を押してください", 40, 145);

  drawIdleRipple();
  drawWifiStatus();
}

function drawWifiStatus() {
  noStroke();
  fill(150);
  textSize(10);
  textAlign(LEFT, TOP);

  text(`ESP32 IP: ${ESP32_IP}`, 40, height - 58);
  text(`Wi-Fi status: ${wifiStatus}`, 40, height - 40);

  if (lastWifiSendTime > 0) {
    const secondsAgo = (millis() - lastWifiSendTime) / 1000;
    text(`last send: ${secondsAgo.toFixed(1)} sec ago`, 40, height - 22);
  }
}
